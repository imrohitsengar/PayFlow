...
const envSchema = z.object({
  ...
  DATABASE_URL: z
    ...
    .min(
      1,
      "DATABASE_URL is required (e.g., postgresql://user:pass@localhost:5432/paytm)",
    ),
  RAZORPAY_KEY_ID: z
    .string()
    .min(
      1,
      "RAZORPAY_KEY_ID is required (get from Razorpay Dashboard → API Keys)",
    ),
  RAZORPAY_KEY_SECRET: z
    .string()
    .min(
      1,
      "RAZORPAY_KEY_SECRET is required (get from Razorpay Dashboard → API Keys)",
    ),
});
